...
void main() {
  ...
  final idRegex = RegExp(r'id:\s*(-?|)\(?((?:bool\.fromEnvironment[^\?]+\?\s*-?\d+\s*:\s*)?)(\d+)\)?');
  ...
  for (final file in files) {
    ...
    content = content.replaceAllMapped(RegExp(r'\(\s*bool\.fromEnvironment\([^?]+\?\s*-?\d+\s*:\s*(-?\d+)\s*\)'), (match) {
      return match.group(1)!;
    });

    final newContent = content.replaceAllMapped(RegExp(r'id:\s*(-?\d+)'), (match) {
      ...
      if (val > jsSafeLimit) {
        ...
        print('Mathematical patch applied in ${file.path.split(Platform.pathSeparator).last}: $sign($high * 1000000000 + $low)');
